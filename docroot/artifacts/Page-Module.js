...
Elm.Component = Elm.Component || {};
Elm.Component.Module = Elm.Component.Module || {};
Elm.Component.Module.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Module = _elm.Component.Module || {};
   if (_elm.Component.Module.values)
   return _elm.Component.Module.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $String = Elm.String.make(_elm),
   $Text = Elm.Text.make(_elm);
   var viewVar = F3(function (innerWidth,
   documentation,
   $var) {
      return function () {
         var _v0 = A2($Dict.get,
         ...
         switch (_v0.ctor)
         {case "Just":
            return A3($Component$Documentation.viewEntry,
              innerWidth,
              $var,
              _v0._0);
            case "Nothing":
            return function () {
                 var msg = $Text.concat(_L.fromArray([$Text.fromString("There is some problem with the docs for \'")
                                                     ,$Text.monospace($Text.fromString($var))
                                                     ,$Text.fromString("\', please inform the package author.")]));
                 return A2($Graphics$Element.width,
                 innerWidth,
                 $Text.leftAligned(msg));
              }();}
         _U.badCase($moduleName,
         "between lines 107 and 119");
      }();
   });
   var viewProse = F2(function (innerWidth,
   prose) {
      return A2($Graphics$Element.width,
      innerWidth,
      A2($Markdown.toElementWith,
   ...
   var viewPair = F3(function (innerWidth,
   documentation,
   _v2) {
      return function () {
         switch (_v2.ctor)
   ...
   var docsPattern = $Regex.regex("(.*)\n@docs\\s+([a-zA-Z0-9_\']+(?:,\\s*[a-zA-Z0-9_\']+)*)");
   var takeWhile = F2(function (pred,
   chunk) {
      return function () {
         var _v6 = $String.uncons(chunk);
         switch (_v6.ctor)
         {case "Just":
            switch (_v6._0.ctor)
              {case "_Tuple2":
                 return pred(_v6._0._0) ? function () {
                      var $ = A2(takeWhile,
                      pred,
                      _v6._0._1),
                      result = $._0,
   ...
   var extractVars = function (rawChunk) {
      return function () {
         var chunk = $String.trimLeft(rawChunk);
         return function () {
            var _v10 = $String.uncons(chunk);
            switch (_v10.ctor)
            {case "Just":
               switch (_v10._0.ctor)
                 {case "_Tuple2":
                    return $Component$Documentation.isVarChar(_v10._0._0) ? function () {
                         var $ = A2(takeWhile,
                         $Component$Documentation.isVarChar,
                         chunk),
                         $var = $._0,
                         rest = $._1;
                         var $ = extractCommaThenVars(rest),
                         vars = $._0,
                         nextChunk = $._1;
                         return {ctor: "_Tuple2"
                                ,_0: A2($List._op["::"],
                         ...
                         op = $._0,
                         rest = $._1;
                         var $ = extractCommaThenVars(A2($String.dropLeft,
                         1,
                         rest)),
                         vars = $._0,
                         nextChunk = $._1;
                         return {ctor: "_Tuple2"
                                ,_0: A2($List._op["::"],op,vars)
                                ,_1: nextChunk};
                      }();}
                 break;
               case "Nothing":
               return {ctor: "_Tuple2"
                      ...
                      ,_1: ""};}
            _U.badCase($moduleName,
            "between lines 46 and 61");
         }();
      }();
   };
   var extractCommaThenVars = function (rawChunk) {
      return function () {
         var chunk = $String.trimLeft(rawChunk);
         return function () {
            var _v14 = $String.uncons(chunk);
            switch (_v14.ctor)
            {case "Just":
               switch (_v14._0.ctor)
                 {case "_Tuple2":
   ...
   var viewDocs = F3(function (innerWidth,
   documentation,
   comment) {
      return function () {
         var _raw = A2($String.split,
         "\n@docs ",
         comment),
         $ = _raw.ctor === "::" ? _raw : _U.badCase($moduleName,
         "on line 31, column 9 to 40"),
         prose = $._0,
         chunks = $._1;
         var varProsePairs = A2($List.map,
         extractVars,
         chunks);
         return $Graphics$Element.flow($Graphics$Element.down)(A2($List._op["::"],
         A2(viewProse,innerWidth,prose),
         A2($List.concatMap,
         A2(viewPair,
         innerWidth,
         documentation),
   ...
   docs) {
      ...
      _L.fromArray([A7($Component$Header.view,
                   ...
                   ,A3(viewDocs,
                   innerWidth,
                   $Component$Documentation.toDocDict(docs),
                   docs.comment)]));
   });
   _elm.Component.Module.values = {_op: _op
                                  ,view: view
                                  ,viewDocs: viewDocs
                                  ,extractVars: extractVars
                                  ,extractCommaThenVars: extractCommaThenVars
                                  ,takeWhile: takeWhile
...
Elm.Page = Elm.Page || {};
Elm.Page.Module = Elm.Page.Module || {};
Elm.Page.Module.make = function (_elm) {
   "use strict";
   _elm.Page = _elm.Page || {};
   _elm.Page.Module = _elm.Page.Module || {};
   if (_elm.Page.Module.values)
   return _elm.Page.Module.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $ColorScheme = Elm.ColorScheme.make(_elm),
   $Component$Documentation = Elm.Component.Documentation.make(_elm),
   $Component$Module = Elm.Component.Module.make(_elm),
   ...
   var moduleNameToUrl = function (name) {
   ...
   function (v) {
      return typeof v === "object" && "user" in v && "name" in v && "version" in v && "versionList" in v && "moduleName" in v ? {_: {}
                                                                                                                                ...
                                                                                                                                v.versionList)
                                                                                                                                ,moduleName: typeof v.moduleName === "string" || typeof v.moduleName === "object" && v.moduleName instanceof String ? v.moduleName : _U.badPort("a string",
                                                                                                                                v.moduleName)} : _U.badPort("an object with fields \'user\', \'name\', \'version\', \'versionList\', \'moduleName\'",
      v);
   });
   ...
   var handleResult = function (response) {
      return function () {
         ...
         {case "Success":
            return function () {
                 var _v2 = A2($Json$Decode.decodeString,
                 $Component$Documentation.documentation,
                 response._0);
                 switch (_v2.ctor)
                 {case "Err":
                    return _U.replace([["comment"
                                       ,"There was an error loading these docs! They may be corrupted."]],
                      dummyDocs);
   ...
   var documentation = $Signal.map(handleResult)($Http.sendGet($Signal.constant(documentationUrl)));
   var docsLoaded = _P.portOut("docsLoaded",
   ...
   $Signal.map(function (v) {
      return A2($Basics._op["++"],
      packageUrl(v),
      A2($Basics._op["++"],
      "/",
      moduleNameToUrl(context.moduleName)));
   })(A3($Signal.keepIf,
   F2(function (x,y) {
      return !_U.eq(x,y);
   })(""),
      ...
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 $Graphics$Element.down,
                 _L.fromArray([$Component$TopBar.view(_v5._0)
                              ,A2($Graphics$Element.flow,
                              $Graphics$Element.right,
                              _L.fromArray([A2($Graphics$Element.spacer,
                                           (_v5._0 - innerWidth) / 2 | 0,
                                           _v5._1 - $Component$TopBar.topBarHeight)
                                           ,A7($Component$Module.view,
   ...
   $Window.dimensions,
   documentation);
   _elm.Page.Module.values = {_op: _op
                             ,packageUrl: packageUrl
                             ,moduleNameToUrl: moduleNameToUrl
